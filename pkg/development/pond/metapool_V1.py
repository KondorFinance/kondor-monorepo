...
class Metapool(Application):
    ...
    pool_token: Final[ApplicationStateValue] = ApplicationStateValue(
        ...
        key=Bytes("p"),
        static=True,
        descr="The asset id of the Pond Token, used to track share of pool the holder may recover",
    )
    ...
    def bootstrap(
        ...
        *,
        ...
        pool token. Note this method will fail if it is attempted more than 
        once on the same contract since the assets and pool token application 
        ...
        Args:
            ...
            in to assets and create pool token.
            a_asset: One of the two assets this pool should allow swapping 
            between.
            b_asset: One of the two assets this pool should allow swapping 
            between.
        Returns:
        ...
        return Seq(
            *Helpers.commented_assert(Txn, well_formed_bootstrap),
            ...
            (una := AssetParam.unitName(self.asset_a)),
            (unb := AssetParam.unitName(self.asset_b)),
            self.pool_token.set(
                Transactions.do_create_token(
                    self,
                    self.asset_a,
                    self.asset_b,
                    Concat(
    ...
    def mint(
        ...
        """mint pool tokens given some amount of a_asset and b_asset 
        on commit.

        Given some amount of stable assets in the transfer, mint some number of 
        pool tokens calculated with the pool's current balance and 
        circulating supply of pool tokens.

        Args:
            a_xfer: Asset Transfer Transaction of a_asset as a deposit to the 
            pool in exchange for pool tokens.
            b_xfer: Asset Transfer Transaction of b_asset as a deposit to the 
            pool in exchange for pool tokens.
            pool_asset: The asset ID of the pool token so that we may 
        ...
        return Seq(
            ...
            *Helpers.commented_assert(
            ...
            # Check that we have these data to calculate pool token amt out
            ...
            # mint tokens
            Transactions.do_axfer(self, Txn.sender(), self.pool_token, to_mint.load()),
